...
export async function getDashboardSummary() {
  ...
  const [projectsRow, leadsRow, reviewRow, wonRow, upcomingDeadlines, recentLeads] =
    await Promise.all([
      db
        .select({ count: sql<number>`count(*)::int` })
        .from(searchResults)
        .where(
          and(
            searchResultNotDeleted(),
            eq(searchResults.verificationStatus, "PENDING"),
          ),
        ),
      db
        .select({ count: sql<number>`count(*)::int` })
      ...
      db
        ...
        .where(
          and(
            opportunityNotDeleted(),
            or(
              eq(opportunities.status, "DETECTED"),
              eq(opportunities.status, "UNDER_REVIEW"),
            ),
          ),
        ),
      db
        ...
        .from(opportunities)
        .where(
          and(opportunityNotDeleted(), eq(opportunities.status, "WON")),
        ),
      db
        .select({
          id: opportunities.id,
        ...
        .where(
          and(
            ...
            isNotNull(opportunities.deadlineAt),
            gte(opportunities.deadlineAt, now),
            lte(opportunities.deadlineAt, inSevenDays),
          ),
        )
  ...
  return {
    pendingProjects: projectsRow[0]?.count ?? 0,
    totalLeads: leadsRow[0]?.count ?? 0,
    leadsInReview: reviewRow[0]?.count ?? 0,
    leadsWon: wonRow[0]?.count ?? 0,
    upcomingDeadlines,
    recentLeads,
  };
}
